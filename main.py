...
def main():
    ...
    parser.add_argument("--device", type=str, choices=["cpu", "gpu"], default="cpu",
                        help="Choose computation device: 'cpu' or 'gpu' (default: cpu)")

    args = parser.parse_args()

    # Setup device context (NEW)
    if args.device == "gpu":
        try:
            import dpctl
            logger.info(" Using GPU for computation...")
            device = dpctl.SyclDevice("gpu")
            logger.info(f" Selected GPU device: {device.name}")
        except Exception as e:
            logger.error(f" GPU device not available: {e}. Falling back to CPU.")
            args.device = "cpu"
    else:
